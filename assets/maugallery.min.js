...
element.wrap(`<div class='item-column mb-4${columnClasses}'></div>`)}else{console.error(`Columns should be defined as numbers or objects. ${typeof columns} is not supported.`)}},moveItemInRowWrapper(element){element.appendTo(".gallery-items-row")},responsiveImageItem(element){if(element.prop("tagName")==="IMG"){element.addClass("img-fluid")}},openLightBox(element,lightboxId){$(`#${lightboxId}`).find(".lightboxImage").attr("src",element.attr("src"));$(`#${lightboxId}`).modal("toggle")},prevImage(){const images=document.querySelectorAll("img.gallery-item");const lightboxImage=document.querySelector(".lightboxImage");if(!lightboxImage)return;let activeImage=null;images.forEach((img)=>{if(img.src===lightboxImage.src){activeImage=img}});if(!activeImage)return;let activeIndex=Array.from(images).indexOf(activeImage);const prevIndex=(activeIndex-1)%images.length;setTimeout(()=>{lightboxImage.src=images[prevIndex].src;console.log("Image updated in lightbox:",lightboxImage.src)},100);let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
let index=0,next=null;$(imagesCollection).each(function(i){if($(activeImage).attr("src")===$(this).attr("src")){index=i}});next=imagesCollection[index]||imagesCollection[imagesCollection.length-1];$(".lightboxImage").attr("src",$(next).attr("src"))},nextImage(){const images=document.querySelectorAll("img.gallery-item");const lightboxImage=document.querySelector(".lightboxImage");if(!lightboxImage)return;let activeImage=null;images.forEach((img)=>{if(img.src===lightboxImage.src){activeImage=img}});if(!activeImage)return;let activeIndex=Array.from(images).indexOf(activeImage);const nextIndex=(activeIndex+1)%images.length;setTimeout(()=>{lightboxImage.src=images[nextIndex].src;console.log("Image updated in lightbox:",lightboxImage.src)},100);let activeTag=$(".tags-bar span.active-tag").data("images-toggle");let imagesCollection=[];if(activeTag==="all"){$(".item-column").each(function(){if($(this).children("img").length){imagesCollection.push($(this).children("img"))}})}else{$(".item-column").each(function(){if($(this).children("img").data("gallery-tag")===activeTag){imagesCollection.push($(this).children("img"))}})}
